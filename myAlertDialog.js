...
export default class AlertDialog extends Gtk.MessageDialog {
  static {
  ...
  constructor({
    ...
    if (buttons?.length > 0) {
      buttons.forEach((label, i) => {
        ...
        if (default_button === i) this.set_default_response(i);
        if (cancel_button === i) this.#cancel_return = i;
      });
    } else {
  ...
  choose(parent, cancellable, callback) {
    ...
    if (cancellable)
      this.#cancellable_signal = cancellable.connect("cancelled", (c) =>
        this._cancelled_cb(c),
      );
  ...
  on_response(response) {
    ...
    if (response == Gtk.ResponseType.CLOSE) {
      this.#task.return_error(
        new GLib.Error(
          GLib.quark_from_string(DIALOG_ERROR_QUARK),
