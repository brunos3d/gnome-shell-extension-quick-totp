...
function makeSchemaTOTP() {
  return new Secret.Schema(KEYRING_SCHEMA_ID, Secret.SchemaFlags.NONE, {
...
function makeSchema(otp) {
  if (otp.type == "TOTP") return makeSchemaTOTP();
  if (otp.type == "HOTP") return makeSchemaHOTP();
...
async function ensureCollectionExists() {
  ...
  await Secret.Collection.create(
    service,
    "OTP",
    null,
...
export async function isOTPCollectionLocked() {
...
export async function lockOTPCollection() {
  const [service, collection] = await findOTPCollection();
  if (!collection) return false;
  return (await service.lock([collection], null)) > 0;
}

export async function unlockOTPCollection() {
  const [service, collection] = await findOTPCollection();
  if (!collection) return false;
...
export async function getOTPItems(unlock = false) {
  try {
    let flags = Secret.SearchFlags.ALL;
    if (unlock) flags |= Secret.SearchFlags.UNLOCK;
    const totp_items = await Secret.password_search(
      makeSchemaTOTP(),
      { type: "TOTP" },
      flags,
      null,
    );
    const hotp_items = await Secret.password_search(
      makeSchemaHOTP(),
      { type: "HOTP" },
      flags,
      null,
...
export async function getOTPItem(otp) {
  let match = makeAttributesFor(otp);
  if (otp.type == "HOTP") delete match.counter; // don't match the counter
  const [item] = await Secret.password_search(
    makeSchema(otp),
    match,
    Secret.SearchFlags.LOAD_SECRETS, // don't unlock
    null,
  );
  if (!item) throw new Error(_("Failed to lookup secret."));
  return item;
}
...
export async function getSecret(otp) {
  const match = makeAttributesFor(otp);
  if (otp.type == "HOTP") delete match.counter; // don't match the counter
  const secret = await Secret.password_lookup(makeSchema(otp), match, null);
  if (secret == null) {
...
export async function updateOTPItem(old_otp, new_otp) {
  const service = await Secret.Service.get(
  ...
  if (old_otp.type == "HOTP") delete match.counter; // don't match the counter
  ...
  if (!item) throw new Error(_("Failed to lookup item."));
...
export async function updateOTPOrder(otp, order) {
  const service = await Secret.Service.get(
    Secret.ServiceFlags.OPEN_SESSION | Secret.ServiceFlags.LOAD_COLLECTIONS,
    null,
  );
  const match = makeAttributesFor(otp);
  if (otp.type == "HOTP") delete match.counter;
  const [item] = await service.search(
    makeSchema(otp),
    match,
...
export async function incrementHOTP(otp) {
  const service = await Secret.Service.get(
    Secret.ServiceFlags.OPEN_SESSION | Secret.ServiceFlags.LOAD_COLLECTIONS,
    null,
  );
  ...
  if (!(await item.set_attributes(makeSchema(otp), new_attr, null)))
    throw new Error(_("Failed to set counter."));
  return new_counter;
}

export async function createOTPItem(otp, order) {
...
export async function removeOTPItem(otp) {
  const match = makeAttributesFor(otp);
  if (otp.type == "HOTP") delete match.counter; // don't match the counter
  return await Secret.password_clear(makeSchema(otp), match, null);
}
